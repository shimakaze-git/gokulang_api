...
class RomajiKanaConvertor:
    ...
    def katakana_to_hiragana(self, text):
        return self.re_kata2hira.sub(
            lambda x: self.kata[x.group(0)],
            text
        )

    def romaji_to_katakana(self, text):
        result = text.lower()
        result = self.rx_mba.sub(r"ン\1\2", result)
        result = self.rx_xtu.sub(r"ッ\1", result)
        # result = self.rx_a__.sub(r"\1ー", result)
        return self.re_roma2kana.sub(
            lambda x: self.romaji_dict[x.group(0)],
            result
        )

    def execute(self, text):
        self.romaji_dict = {}
        for tbl in self.master, self.romaji_asist:
            for k, v in tbl.items():
                self.romaji_dict[k] = v

        romaji_keys = self.romaji_dict.keys()
        # romaji_keys.sort(key=lambda x:len(x), reverse=True)
        romaji_keys = sorted(romaji_keys, key=lambda x: len(x), reverse=True)

        self.re_roma2kana = re.compile("|".join(map(re.escape, romaji_keys)))
        self.rx_mba = re.compile("m(b|p)([aiueo])")
        self.rx_xtu = re.compile(r"([bcdfghjklmpqrstvwxyz])\1")
        self.rx_a__ = re.compile(r"([aiueo])\1")

        result = self.romaji_to_katakana(text)
        hiragana_text = self.katakana_to_hiragana(result)
        return hiragana_text, result


# if __name__ == "__main__":
#     # _romaji_to_hiragana
#     romaji_kana_convertor = RomajiKanaConvertor()
#     text = romaji_kana_convertor.execute("ukulele")
#     print(text)
